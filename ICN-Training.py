    ...
    for epoch in pbar:
        ...
        with torch.no_grad():
            ...
            if epoch % 5 == 0:
                ...
                print('y:{}\nyhat:{}'.format(y,ŷ))
                debug_graph['org'] = y[0]
                debug_graph['predict'] = ŷ[0].squeeze()
        ...
        if es > config.early_stopping:
            ...
            break
        es = es + 1
    fig, ax = analysis.plot_predictions(debug_graph)
    fig.savefig(os.path.join(config.save_path, 'debug_graph.png'))
...
def main():
    ...
    # Series data
    train_x, train_y = icn_data.create_lstm_dataset(train_data[config.numeric_cols], config.target_cols, config.training_win_size, config.prediction_win_size)
    val_x, val_y = icn_data.create_lstm_dataset(val_data[config.numeric_cols], config.target_cols, config.training_win_size, config.prediction_win_size)
    ...
    model_params = {
        ...
        'hidden_dim' : 64,
        'output_dim' : 1,
        'output_size' : config.prediction_win_size,
        'layers' : 3
    }
    dense_model_params = {
    ...
    with torch.no_grad():
        #predict['Dense'] = dense_model(torch.flatten(predict_x, start_dim=1)[0]).squeeze()
